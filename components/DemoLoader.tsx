...
export default function DemoLoader({ onComplete }: DemoLoaderProps) {
    ...
    const [progNum, setProgNum] = useState("00");
    ...
    useEffect(() => {
        ...
        const tick = () => {
            if (done) return;
            const raw = Math.min(((Date.now() - start) / (ARDENO_DUR * 0.85)) * 100, 100);
            setProgNum(String(Math.round(raw)).padStart(2, "0"));
        ...
        // Phase 2 in — mount early so it's ready before Phase 1 fully fades
    ...
    const sharedPhaseStyle: React.CSSProperties = {
    ...
    return (
        ...
        @keyframes charIn     { from{opacity:0;transform:translateY(14px);filter:blur(5px)} to{opacity:1;transform:translateY(0);filter:blur(0)} }
        ...
        @keyframes dotSlide   { from{left:0%} to{left:100%} }
        @keyframes shimmer    { from{left:-14px} to{left:calc(100% - 14px)} }
        @keyframes flashRed   { 0%{opacity:0} 40%{opacity:1} 100%{opacity:0} }
        @keyframes fadeIn     { from{opacity:0} to{opacity:1} }
        @keyframes fadeOut    { from{opacity:1;transform:scale(1)} to{opacity:0;transform:scale(1.04)} }
        @keyframes revealUp   { from{opacity:0;transform:translateY(-14px) scale(.92)} to{opacity:1;transform:translateY(0) scale(1)} }
      ...
      ══════════════════════════════════════════ */}
            <div
                style={{
                    ...
                    zIndex: 3,
                    animation: p1Out ? "fadeOut .8s cubic-bezier(.4,0,.2,1) forwards" : undefined,
                }}
            >
                {/* Grain */}
                ...
                {/* Ambient breathe — gold */}
                <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 45%,rgba(212,184,150,.1) 0%,transparent 55%)", animation: "breathe 5s ease-in-out infinite", pointerEvents: "none" }} />
                {/* Flash overlay — gold */}
                {p1Flash && (
                    <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 50% 50%,rgba(212,184,150,.15) 0%,transparent 80%)", animation: "flashRed .6s cubic-bezier(.16,1,.3,1) forwards", pointerEvents: "none", zIndex: 10 }} />
                )}

                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                    {/* Ardeno A mark — gold */}
                    <div style={{ width: 80, height: 80, marginBottom: 8, opacity: 0, animation: "revealUp 1.4s cubic-bezier(.16,1,.3,1) .1s forwards" }}>
                        <svg viewBox="200 580 360 340" style={{ overflow: "visible" }}>
                            <path d={ARDENO_PATH} fill="none" stroke="rgba(212,184,150,0.08)" strokeWidth={1} />
                            <path d={ARDENO_PATH} fill="none" stroke="url(#gStroke)" strokeLinecap="round" strokeWidth={3.5} style={{ strokeDasharray: 2000, animation: "drawPath 2.2s cubic-bezier(.2,1,.4,1) .4s forwards" }} />
                            <path d={ARDENO_PATH} fill="url(#gGrad)" filter="url(#gGlow)" style={{ opacity: 0, transformOrigin: "center", animation: "fillFade 1.4s cubic-bezier(.16,1,.3,1) 1.8s forwards" }} />
                        </svg>
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                        ...
                        />
                        <div style={{ height: 1, width: 40, background: "rgba(214,193,146,.2)", transformOrigin: "center", animation: "scaleX .9s cubic-bezier(.22,1,.36,1) 1.8s forwards", opacity: 0, marginTop: 4 }} />
                        <StaggerText
                            text="presents"
                            baseDelay={1.9}
                            animation="charIn"
                            charStyle="font-family:'Cormorant Garamond',serif;font-size:13px;font-weight:300;font-style:italic;color:rgba(214,193,146,.6);letter-spacing:.22em"
                        />
                    </div>
                </div>

                {/* Progress bar — website style, bottom edge, gold */}
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 2, background: "rgba(212,184,150,.1)" }}>
                    <div style={{ height: "100%", background: "linear-gradient(90deg,#c8a87e,#f0dfc3)", transformOrigin: "left", animation: "progFill 2.5s cubic-bezier(.16,1,.3,1) forwards", boxShadow: "0 0 12px rgba(212,184,150,.8),0 0 24px rgba(212,184,150,.4)" }} />
                    <div style={{ position: "absolute", top: "50%", transform: "translateY(-50%)", width: 6, height: 6, borderRadius: "50%", background: "#f0dfc3", boxShadow: "0 0 8px rgba(212,184,150,1),0 0 16px rgba(212,184,150,.6)", animation: "dotSlide 2.5s cubic-bezier(.16,1,.3,1) forwards" }} />
                </div>
            </div>
            ...
            {showReplay && (
                <button
                    ...
                    style={{
                        ...
                        background: "rgba(214,193,146,.08)", border: "1px solid rgba(214,193,146,.25)",
                        color: "rgba(214,193,146,.6)", padding: "9px 28px",
                        fontFamily: "Sora,sans-serif", fontSize: 11, letterSpacing: ".2em",
                        cursor: "pointer", zIndex: 99999, backdropFilter: "blur(6px)",
                    }}
                >
                    ↺ &nbsp; Replay
                </button>
